...
- 홈에서 카드형·목록형 보기를 전환할 수 있습니다. 전체 경매와 검색 결과 화면은 검색창 아래에 경매 상태 선택을 바로 표시하고, 필터·정렬·카드형/목록형 전환을 한 행에 둡니다. 카테고리·가격은 필터 시트에서 선택한 뒤 `결과 보기`를 눌러 적용하며, 적용된 조건은 칩에서 각각 해제할 수 있습니다. 결과 수는 현재 표시된 경매 개수로 안내합니다.
...
6. 전체 경매에서 상태를 바꾸고 정렬 메뉴와 카드형/목록형 전환을 사용합니다. 필터 시트의 선택을 적용하지 않고 닫으면 기존 결과가 유지되고, `결과 보기`를 누르면 카테고리·가격 조건이 함께 적용되는지 확인합니다.
7. 적용된 조건 칩을 하나씩 해제하고 추가 페이지를 불러올 때 `현재 n개 표시`가 화면에 표시된 경매 개수와 일치하는지 확인합니다.
